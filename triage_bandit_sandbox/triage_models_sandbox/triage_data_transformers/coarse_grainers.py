from typing import List, Optional
import numpy as np

from babylon_pgm.constants import LabelType
from babylon_pgm.triage_models.triage_decisions import (
    TRIAGE_MODEL_DECISION_TYPE,
    TriageModelDecisionDefault,
)
from .base_data_extractor import BaseDataExtractor
from ..interfaces import TriageDataset, PreparedData


class TriageDiseasesCoarseGrainer(BaseDataExtractor):
    """
    Convert a TriageDataset object into a PreparedData object.
    Diseases differentials are included in the features of the triage model
    after coarse-graining them according to their optimal triage decision.
    Symptoms and risks differentials are included in the features
    without coarse-graining them.

    :param triage_decisions: Triage decisions to be used (according to world region).
    :param node_types: List of node types whose differential
                        will be included in the triage model features.
    :param include_age: Whether to include age or not in the triage model features.
    """
    def __init__(
            self,
            triage_decisions: Optional[TRIAGE_MODEL_DECISION_TYPE] = TriageModelDecisionDefault,
            node_types: Optional[List[LabelType]] = [LabelType.disease],
            include_age: Optional[bool] = True,
    ):
        self._triage_decisions = triage_decisions
        self.node_types = [t for t in node_types if t != LabelType.disease]
        self.include_age = include_age
        self.features_id = [k.value for k in self._triage_decisions]
        self.num_features = len(self.features_id)

    def fit(self, dataset: TriageDataset):
        """
        Fit the parameters used to transform the data.
        """
        if len(self.node_types) > 0:
            set_of_nodes_id = set()
            for card in dataset:
                card_nodes_id = [
                    d.node.id for d in card.differentials
                    if d.node.label in self.node_types
                ]
                set_of_nodes_id = set_of_nodes_id.union(set(card_nodes_id))

            self.features_id.extend(list(set_of_nodes_id))
            self.num_features += len(set_of_nodes_id)

    def transform(self, dataset: TriageDataset, train: bool) -> PreparedData:
        """
        Transform the data.

        :param dataset: Data to be transformed.
        :param train: Whether the data are used to train the model.
        :return: PreparedData object containing only features
                    (differentials of nodes included in self.node_types
                    and coarse-grained differentials of disease nodes)
                    and correct_decisions.
        """
        if train:
            correct_decisions, dataset = self._extract_correct_decisions(dataset)
        else:
            correct_decisions = None
        ages_array = np.zeros((len(dataset), 1))
        X = np.zeros((len(dataset), self.num_features))
        for i, card in enumerate(dataset):
            ages_array[i] = card.age / 100
            card_dict = {k: 0. for k in self.features_id}
            for d in card.differentials:
                if d.probability is not None:
                    if d.node.label == LabelType.disease:
                        card_dict[d.node.triage] += d.probability
                    else:
                        try:
                            card_dict[d.node.id] = d.probability
                        except KeyError:
                            pass
            X[i] = [card_dict.get(node_id, 0) for node_id in self.features_id]
            normalisation_const = np.sum(X[i, :len(self._triage_decisions)])
            if normalisation_const > 0:
                X[i, :len(self._triage_decisions)] /= normalisation_const
        if self.include_age:
            X = np.concatenate((X, ages_array), axis=1)
        return PreparedData(
            features=X,
            correct_decisions=correct_decisions,
        )

    def fit_transform(self, dataset: TriageDataset, train: bool) -> PreparedData:
        """
        Fit the parameters for the data transformation and transform the data.
        (Use this method as default to transform data).

        :param dataset: Data to be transformed.
        :param train: Whether the data are used to train the model.
        :return: PreparedData object containing only features
                    (differentials of nodes included in self.node_types
                    and coarse-grained differentials of disease nodes)
                    and correct_decisions.
        """
        self.fit(dataset)
        return self.transform(dataset, train)




class CrueltyDiseasesCoarseGrainer(TriageDiseasesCoarseGrainer):
    """
    Convert a TriageDataset object into a PreparedData object.
    Diseases differentials are included in the features of the triage model
    after coarse-graining them according to their cruelty distribution.
    Symptoms and risks differentials are included in the features
    without coarse-graining them.

    :param triage_decisions: Triage decisions to be used (according to world region).
    :param node_types: List of node types whose differential
                        will be included in the triage model features.
    :param include_age: Whether to include age or not in the triage model features.
    """
    def __init__(
            self,
            triage_decisions: Optional[TRIAGE_MODEL_DECISION_TYPE] = TriageModelDecisionDefault,
            node_types: Optional[List[LabelType]] = [LabelType.disease],
            include_age: Optional[bool] = True,
    ):
        self._triage_decisions = triage_decisions
        self.node_types = [t for t in node_types if t != LabelType.disease]
        self.include_age = include_age
        self.features_id = list(np.arange(1, 8))
        self.num_cruelties = len(self.features_id)
        self.num_features = len(self.features_id)

    def transform(self, dataset: TriageDataset, train: bool) -> PreparedData:
        """
        Transform the data.

        :param dataset: Data to be transformed.
        :param train: Whether the data are used to train the model.
        :return: PreparedData object containing only features
                    (differentials of nodes included in self.node_types
                    and coarse-grained differentials of disease nodes)
                    and correct_decisions.
        """
        if train:
            correct_decisions, dataset = self._extract_correct_decisions(dataset)
        else:
            correct_decisions = None
        ages_array = np.zeros((len(dataset), 1))
        X = np.zeros((len(dataset), self.num_features))
        for i, card in enumerate(dataset):
            ages_array[i] = card.age / 100
            card_dict = {k: 0. for k in self.features_id[7:]}
            for d in card.differentials:
                if d.probability is not None:
                    if d.node.cruelty_distribution is not None:
                        X[i, :self.num_cruelties] += d.probability * np.asarray(d.node.cruelty_distribution)
                    elif len(card_dict) > 0:
                        try:
                            card_dict[d.node.id] = d.probability
                        except KeyError:
                            pass
            X[i, self.num_cruelties:] = [
                card_dict.get(node_id, 0) for node_id in self.features_id[self.num_cruelties:]
            ]
            normalisation_const = np.sum(X[i, :self.num_cruelties])
            if normalisation_const > 0:
                X[i, :self.num_cruelties] /= normalisation_const
        if self.include_age:
            X = np.concatenate((X, ages_array), axis=1)
        return PreparedData(
            features=X,
            correct_decisions=correct_decisions,
        )


